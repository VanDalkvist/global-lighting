...
namespace PathTracing
{
	public class PathTracing
	{
		...
		private Radiance Radiance(HitPoint hitPoint, Vector point, Vector direction)
		{
			...
			while (true)
			{
				...
				//private delegate void CycleDelegate(ref Vector start, ref Vector direction, ref HitPoint hp, ref int i);
				result += factor * direct;
				//break;
				//Compute indirect luminancy
				//int countOfTreads = 10;
		...
		public void ComputeImage(int countFrame)
		{
			...
			{
				// Parallel
				Parallel.For(0, countOfPixelsOnHorizontal, i => { Cycle(ref i); });
			}

			//Parallel.For(i, i + countOfPixelsOnHorizontal/countOfTreads, new System.Action<int>(Cycle))
			for (int i = 0; i < countOfPixelsOnHorizontal; i++)
				for (int j = 0; j < countOfPixelsOnVertical; j++)
					arrayOfRadiances[i, j] /= countFrame;
		}

		private void Cycle(ref int i)
		{
			double px;
			double py;
			double distance = 1;
			...
			for (int j = 0; j < countOfPixelsOnVertical; j++)
			{
				...
				if (hp != null)
				{
					...
					arrayOfRadiances[i, j] += Radiance(hp, start, direction);
				}
			}
		}
